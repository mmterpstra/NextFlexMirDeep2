...
quantifier.pl -g 0 -c ${mainDir}/nf.sample.config -p ${mainDir}/mirbase21/high_conf_hairpin.fa -m ${mainDir}/mirbase21/high_conf_mature.fa -r $(pwd)/high_conf_nf.sample.all.reads.fa -y nf.highconf -P -t hsa -W
...
quantifier.pl -g 0 -c ${mainDir}/default.sample.config -p ${mainDir}/mirbase21/high_conf_hairpin.fa -m ${mainDir}/mirbase21/high_conf_mature.fa -r $(pwd)/high_conf_default.sample.all.reads.fa -y default.highconf -P -t hsa -W

mkdir -p $outDir/nf
cd $outDir/nf
perl -wne 'my $header = $_; defined(my $seq = <>) or die "Input error incomplete fasta"; chomp $seq;print $header.$seq."\n" if(length($seq) >= 17);' ${mainDir}/nf.sample.all.reads.fa > $(pwd)/nf.sample.all.reads.fa
quantifier.pl -g 0 -c ${mainDir}/nf.sample.config -p ${mainDir}/mirbase21/hairpin.fa -m ${mainDir}/mirbase21/mature.fa -r $(pwd)/nf.sample.all.reads.fa -P -t hsa -W -y nf

mkdir -p $outDir/default
cd $outDir/default
perl -wne 'my $header = $_; defined(my $seq = <>) or die "Input error incomplete fasta"; chomp $seq;print $header.$seq."\n" if(length($seq) >= 17);' ${mainDir}/default.sample.all.reads.fa > $(pwd)/default.sample.all.reads.fa
quantifier.pl -g 0 -c ${mainDir}/default.sample.config -p ${mainDir}/mirbase21/hairpin.fa -m ${mainDir}/mirbase21/mature.fa -r $(pwd)/default.sample.all.reads.fa -P -t hsa -W -y default
